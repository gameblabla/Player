...
#define setPixel(x,y,c) *((uint32_t* )vram_l + ((x) + (y) * (320))) = c;
...
static void drawChar(int32_t *x, int32_t *y, int32_t margin, char ch, uint32_t fc, uint32_t olc)
{
	int32_t i, j;
	uint8_t *charSprite;
	if (ch == '\n')
...
static void drawString(int32_t *x, int32_t *y, int32_t _x, const char *str, uint32_t fc, uint32_t olc)
{
...
void print_string(const char *s,const uint32_t fg_color, const uint32_t bg_color, int32_t x, int32_t y) 
{
